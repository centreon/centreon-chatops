...
{
    protected $commandFormat = '([\w_-]+)\s+([\w_-]+)\s+(sticky\s+|notify\s+){0,2}(.*)';
    protected $commands = array('service', 'svc');
    protected $subcommand = true;
    protected $help = '_hostname_ _servicename_ [sticky] [notify] _message_';
    ...
    public function __construct($module)
    {
        ...
        Acknowledge::registerSubcommand('service', $this);
        Acknowledge::registerSubcommand('svc', $this);
    }
    ...
    public function run()
    {
      ...
      /* Alternative fix for multiple option */
        $arguments = $this->getArguments($hostname . ' ' . $service, $comment);
      ...
      /* Find sticky on notify options */
        ...
        $extCmdLine = 'ACKNOWLEDGE_SVC_PROBLEM;' . $hostname . ';' . $service . ';';
